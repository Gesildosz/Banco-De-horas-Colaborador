    ...
    const { error: entryError } = await supabase.from("time_entries").insert([
      {
        ...
        balance_hours: balanceHoursChange, // mudança de saldo desta linha
        hours_change: balanceHoursChange, // compatibilidade com colunas alternativas
        new_balance: newBalance, // saldo acumulado após este lançamento
        entry_type,
        description,
      },
    ])
